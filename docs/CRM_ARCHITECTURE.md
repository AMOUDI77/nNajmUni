...
The additive Alembic revision `0002_crm_v1` creates staff, contact identity, conversation, message, attribution, automation, AI, knowledge and durable job tables. The revision uses the frozen `crm/schema_v1.py` contract. Revision `0003_saved_replies` adds reusable staff replies without changing that frozen contract or any legacy table. Its downgrade removes only saved replies. CRM downgrades are destructive to CRM data and are not a routine production rollback.
...
The React CRM shell lives under `client/src/crm` and uses `/crm/*` routes in the existing application. Its five primary areas are Inbox, Automations, CRM, Analytics and Settings. Its typed API client centralizes cookie authentication, CSRF and errors. The inbox has server-paginated conversations/messages, per-conversation session drafts, a concise contact summary, private notes, assignment and takeover. Typing `/` opens searchable saved replies; choosing one only inserts editable text and replaces the three supported profile variables. Mobile collapses to list/thread navigation and a contact drawer. Text uses React escaping and `dir="auto"` for mixed Arabic/English content.
